...
import logging
DLLPATH = base_dir.decode('gbk') + '\\QdUtils.dll'
...
def SetSysVariantValue(vname, value ,Var, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm):
    ...
    return Var, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm
...
def SetSysVariantValueForOrderItem(varstr, newpoi, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm):
    #global value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm
    ...
    if des=='' or (not os.path.exists(dataDBfile)):
        ...
        if des2wuliaoallu==[]:
            ...
            for i in range(0, len(des2wuliaoallu)):
                ...
                key = s1+','+s2
                cjo['key'] = key
                string = fjo['封边留空1'.encode('gbk')]+','+fjo['封边留空2'.encode('gbk')]
                cjo['s1'] = key
                cjo['s2'] = string
                cjo['s3'] = fjo['五金清单1'.encode('gbk')]+','+fjo['五金清单2'.encode('gbk')]
                cjo['direct'] = 0
                cjo['no'] = fjo['输出编号'.encode('gbk')]  # 输出编号
                cjo['oname'] = fjo['输出名称'.encode('gbk')]  # 输出名称
                cjo['childbom'] = fjo['物料分解'.encode('gbk')]  # 物料分解
                cjo['priceclass'] = fjo['type'.encode('gbk')]  # type
                if gQDBomFlag == 0:
                    cjo['bomclass'] = fjo['type'.encode('gbk')]
                else:
                    cjo['bomclass'] = fjo['bomclass'.encode('gbk')]  # bomclass
                cjo['linecalc'] = fjo['线性物体计算'.encode('gbk')]  # 线性物体计算
                cjo['HoleConfig'] = fjo['孔位配置'.encode('gbk')]  # 孔位配置
                cjo['bomstd'] = fjo['尺寸判定'.encode('gbk')]  # 尺寸判定
                cjo['bomtype'] = fjo['物料分类'.encode('gbk')]  # 物料分类
                cjo['KCConfig'] = fjo['开槽配置'.encode('gbk')]  # 开槽配置
                cjo['bg_filename'] = fjo['bg_filename'.encode('gbk')] # bg_filename
                cjo['mpr_filename'] = fjo['mpr_filename'.encode('gbk')]
                cjo['bpp_filename'] = fjo['bpp_filename'.encode('gbk')]
                cjo['devcode'] = fjo['devcode'.encode('gbk')]
                cjo['zero_y'] = fjo['zero_y'.encode('gbk')]
                cjo['is_output_bgdata'] = fjo['is_output_bgdata'.encode('gbk')]
                cjo['is_output_mpr'] = fjo['is_output_mpr'.encode('gbk')]
                cjo['is_output_bpp'] = fjo['is_output_bpp'.encode('gbk')]
                cjo['direct_calctype'] = fjo['direct_calctype'.encode('gbk')]
                cjo['youge_holecalc'] = fjo['youge_holecalc'.encode('gbk')]
                cjo['workflow'] = fjo['workflow'.encode('gbk')]
                ja.append(cjo)
        jo['des2wuliao'] = ja
        ...
        sql = ("select * from bom_rule where deleted=False and 物料类别1='%s' and 物料类别2='%s' order by 物料类别1 asc, 物料类别2 asc, 材料 asc " % (
            fjo['封边留空1'.encode('gbk')].decode('gbk'), fjo['封边留空2'.encode('gbk')].decode('gbk'))).encode('gbk')
        cur.execute(sql)
        ...
        if ruleHashuf != []:
            ...
            for i in range(0, len(ruleHashuf)):
                ...
                cjo = {}
                key = ruleHash_dict['物料类别1'.encode('gbk')]+ ',' + ruleHash_dict['物料类别2'.encode('gbk')]
                cjo['key'] = key
                cjo['myclass1'] = ruleHash_dict['物料类别1'.encode('gbk')]
                cjo['myclass2'] = ruleHash_dict['物料类别1'.encode('gbk')]
                cjo['mat'] = ruleHash_dict['材料'.encode('gbk')]
                cjo['lfb'] = Nonetonumber(ruleHash_dict['l封边'.encode('gbk')])
                cjo['llk'] = Nonetonumber(ruleHash_dict['l留空'.encode('gbk')])
                cjo['wfb'] = Nonetonumber(ruleHash_dict['w封边'.encode('gbk')])
                cjo['wlk'] = Nonetonumber(ruleHash_dict['w留空'.encode('gbk')])
                cjo['holestr'] = ruleHash_dict['打孔'.encode('gbk')]
                cjo['kcstr'] = ruleHash_dict['开槽'.encode('gbk')]
                cjo['memo'] = ruleHash_dict['备注'.encode('gbk')]
                cjo['fbstr'] = ruleHash_dict['封边'.encode('gbk')]
                cjo['is_outline'] =0 if ruleHash_dict['is_outline'.encode('gbk')] =='' else int(ruleHash_dict['is_outline'.encode('gbk')])
                cjo['bh'] = Nonetonumber(ruleHash_dict['bh'.encode('gbk')])

                cjo['llfb'] = Nonetonumber(ruleHash_dict['lfb'.encode('gbk')])
                cjo['rrfb'] = Nonetonumber(ruleHash_dict['rfb'.encode('gbk')])
                cjo['ddfb'] = Nonetonumber(ruleHash_dict['dfb'.encode('gbk')])
                cjo['uufb'] = Nonetonumber(ruleHash_dict['ufb'.encode('gbk')])
                cjo['fb'] = Nonetonumber(ruleHash_dict['fb'.encode('gbk')])
        ...
        sql = "select base_bom2_class.name as bclass, bom2_rule.* from base_bom2_class,bom2_rule where bom2_rule.pid=base_bom2_class.id and bom2_rule.deleted = False and base_bom2_class.name='%s'" % (
            fjo['物料分解'.encode('gbk')].decode('gbk'))
        cur.execute(sql)
        ...
        if wuliao_listuf != []:
            ...
            for i in range(0, len(wuliao_listuf)):
                ...
                cjo = {}
                cjo['key'] =  wuliao_dict['bclass'.encode('gbk')]
                cjo['id'] = Nonetonumber(wuliao_dict['id'.encode('gbk')])
                cjo['name'] = wuliao_dict['名称'.encode('gbk')]
                cjo['mat'] = wuliao_dict['材料'.encode('gbk')]
                cjo['color'] = wuliao_dict['颜色'.encode('gbk')]
                cjo['lfb'] = Nonetonumber(wuliao_dict['l封边'.encode('gbk')])
                cjo['llk'] = Nonetonumber(wuliao_dict['l留空'.encode('gbk')])
                cjo['wfb'] = Nonetonumber(wuliao_dict['w封边'.encode('gbk')])
                cjo['wlk'] = Nonetonumber(wuliao_dict['w留空'.encode('gbk')])
                cjo['holestr'] = wuliao_dict['打孔'.encode('gbk')]
                cjo['kcstr'] = wuliao_dict['开槽'.encode('gbk')]
                cjo['memo'] = wuliao_dict['备注'.encode('gbk')]
                cjo['ono'] = wuliao_dict['输出编号'.encode('gbk')]

                cjo['bclass'] = wuliao_dict['bclass'.encode('gbk')]
                cjo['fbstr'] = wuliao_dict['封边'.encode('gbk')]
                cjo['bomstd'] = wuliao_dict['尺寸判定'.encode('gbk')]
                cjo['bomtype'] = wuliao_dict['物料分类'.encode('gbk')]
                cjo['a_face'] = wuliao_dict['a面孔'.encode('gbk')]
                cjo['b_face'] = wuliao_dict['b面孔'.encode('gbk')]
                cjo['bg_filename'] = wuliao_dict['bg_filename'.encode('gbk')]
                cjo['mpr_filename'] = wuliao_dict['mpr_filename'.encode('gbk')]
                cjo['bpp_filename'] = wuliao_dict['bpp_filename'.encode('gbk')]
                cjo['devcode'] = wuliao_dict['devcode'.encode('gbk')]
                cjo['direct_calctype'] = Nonetonumber(wuliao_dict['direct_calctype'.encode('gbk')])
                cjo['workflow'] = wuliao_dict['workflow'.encode('gbk')]

                cjo['l'] = wuliao_dict['长度'.encode('gbk')]
                cjo['p'] = wuliao_dict['宽度'.encode('gbk')]
                cjo['h'] = wuliao_dict['厚度'.encode('gbk')]
                cjo['q'] = wuliao_dict['数量'.encode('gbk')]

                cjo['llfb'] = Nonetonumber(wuliao_dict['lfb'.encode('gbk')])
                cjo['rrfb'] = Nonetonumber(wuliao_dict['rfb'.encode('gbk')])
                cjo['ddfb'] = Nonetonumber(wuliao_dict['dfb'.encode('gbk')])
                cjo['uufb'] = Nonetonumber(wuliao_dict['ufb'.encode('gbk')])
                cjo['fb'] = Nonetonumber(wuliao_dict['fb'.encode('gbk')])

                cjo['lmax'] = Nonetonumber(wuliao_dict['lmax'.encode('gbk')])
                cjo['lmin'] = Nonetonumber(wuliao_dict['lmin'.encode('gbk')])
                cjo['dmax'] = Nonetonumber(wuliao_dict['dmax'.encode('gbk')])
                cjo['dmin'] = Nonetonumber(wuliao_dict['dmin'.encode('gbk')])
                cjo['hmax'] = Nonetonumber(wuliao_dict['hmax'.encode('gbk')])
                cjo['hmin'] = Nonetonumber(wuliao_dict['hmin'.encode('gbk')])
                cjo['deleted'] = False
                ja.append(cjo)
        jo['childbomHash'] = ja
        #五金
        sql = ("select base_wj_rule.*,[单位],xzw_basewj.[输出编号] as wjno,xzw_basewj.ID as wjid,xzw_basewj.[价格] " \
            ...
            fjo['五金清单1'.encode('gbk')].decode('gbk'), fjo['五金清单2'.encode('gbk')].decode('gbk'))).encode('gbk')
        cur.execute(sql)
        ja = []
        ...
        if wujin_listuf:
            ...
            for i in range(0, len(wujin_listuf)):
                ...
                cjo = {}
                key = wujin_dict['五金规则类别1'.encode('gbk')] + ',' + wujin_dict['五金规则类别2'.encode('gbk')]
                cjo['key'] = key
                cjo['myclass1'] = wujin_dict['五金规则类别1'.encode('gbk')]
                cjo['myclass2'] = wujin_dict['五金规则类别2'.encode('gbk')]
                cjo['lmax'] = Nonetonumber(wujin_dict['宽最大'.encode('gbk')])
                cjo['lmin'] = Nonetonumber(wujin_dict['宽最小'.encode('gbk')])
                cjo['pmax'] = Nonetonumber(wujin_dict['深最大'.encode('gbk')])
                cjo['pmin'] = Nonetonumber(wujin_dict['深最小'.encode('gbk')])
                cjo['hmax'] = Nonetonumber(wujin_dict['高最大'.encode('gbk')])
                cjo['hmin'] = wujin_dict['高最小'.encode('gbk')]
                cjo['wjname'] = wujin_dict['五金名称'.encode('gbk')]
                cjo['wjno'] = wujin_dict['wjno'.encode('gbk')]
                cjo['num'] = Nonetonumber(wujin_dict['数量'.encode('gbk')])
                cjo['myunit'] = wujin_dict['配置规则'.encode('gbk')]
                cjo['lgflag'] = Nonetonumber(wujin_dict['连柜功能'.encode('gbk')])

                cjo['myunit2'] = wujin_dict['单位'.encode('gbk')]
                cjo['mat'] = wujin_dict['材料'.encode('gbk')]
                cjo['color'] = wujin_dict['颜色'.encode('gbk')]
                cjo['wjid'] = Nonetonumber(wujin_dict['wjid'.encode('gbk')])
                cjo['price'] = Nonetonumber(wujin_dict['price'.encode('gbk')])
                ja.append(cjo)
        jo['wjruleHash'] = ja
        sql = ("select * from base_linecalc where name='%s' " % (fjo['线性物体计算'.encode('gbk')].decode('gbk'))).encode('gbk')
        cur.execute(sql)
        ja = []
        ...
        if linecal_listuf:
            ...
            for i in range(0, len(linecal_listuf)):
                ...
                if linecal_dict['name']:
                    cjo['name'] = linecal_dict['name'.encode('gbk')]
                    cjo['linemax'] = Nonetonumber(linecal_dict['linemax'.encode('gbk')])
                ja.append(cjo)
        jo['linecalcList'] = ja

        ja = []
        sql = ("select * from base_holeconfig where name='%s' order by id" % (fjo['孔位配置'.encode('gbk')].decode('gbk'))).encode('gbk')
        cur.execute(sql)
        ...
        if holeconfig_listuf:
            ...
            for i in range(0, len(holeconfig_listuf)):
                ...
                cjo['name'] = holeconfig_dict['name'.encode('gbk')]
                cjo['flag'] = holeconfig_dict['flag']
                cjo['flag2'] = holeconfig_dict['flag2'.encode('gbk')]
                cjo['l_bigname'] = holeconfig_dict['l_bigname']
                cjo['l_smallname'] = holeconfig_dict['l_smallname'.encode('gbk')]
                cjo['i_name'] = holeconfig_dict['i_name']
                cjo['mx_name'] = holeconfig_dict['mx_name'.encode('gbk')]
                cjo['l_holedepth'] = Nonetonumber(holeconfig_dict['l_holedepth'])
                cjo['l_bigcap'] = Nonetonumber(holeconfig_dict['l_bigcap'.encode('gbk')])
                cjo['l_smallcap'] = Nonetonumber(holeconfig_dict['l_smallcap'])
                cjo['calctype'] = Nonetonumber(holeconfig_dict['calctype'.encode('gbk')])
                cjo['holecap'] = holeconfig_dict['holecap']
                cjo['mx_calctype'] = Nonetonumber(holeconfig_dict['mx_calctype'.encode('gbk')])
                cjo['mx_cap'] = Nonetonumber(holeconfig_dict['mx_cap'])
                cjo['l_isoutput'] = Nonetonumber(holeconfig_dict['l_isoutput'.encode('gbk')])
                cjo['i_isoutput'] = Nonetonumber(holeconfig_dict['i_isoutput'])
                cjo['mx_isoutput'] = Nonetonumber(holeconfig_dict['mx_isoutput'.encode('gbk')])
                cjo['ismirror'] = Nonetonumber(holeconfig_dict['ismirror'])
                cjo['iscalc'] = Nonetonumber(holeconfig_dict['iscalc'.encode('gbk')])
                cjo['bigface'] = Nonetonumber(holeconfig_dict['bigface'])
                cjo['myface'] = Nonetonumber(holeconfig_dict['myface'.encode('gbk')])
                cjo['min'] = Nonetonumber(holeconfig_dict['min'])
                cjo['max'] = Nonetonumber(holeconfig_dict['max'.encode('gbk')])
                cjo['bh'] = Nonetonumber(holeconfig_dict['bh'])
                cjo['isoffset'] = Nonetonumber(holeconfig_dict['isoffset'.encode('gbk')])
                cjo['xo'] = holeconfig_dict['xo']
                cjo['yo'] = holeconfig_dict['yo'.encode('gbk')]
                cjo['b_isoffset'] = Nonetonumber(holeconfig_dict['b_isoffset'])
                cjo['b_xo'] = holeconfig_dict['b_xo'.encode('gbk')]
                cjo['b_yo'] = holeconfig_dict['b_yo']
                cjo['pkcap'] = Nonetonumber(holeconfig_dict['pkcap'.encode('gbk')])
                cjo['holenum'] = Nonetonumber(holeconfig_dict['holenum'])
                cjo['center_holenum'] = Nonetonumber(holeconfig_dict['center_holenum'.encode('gbk')])
                cjo['center_holecap'] = holeconfig_dict['center_holecap']

                cjo['i_offsetvalue'] = Nonetonumber(holeconfig_dict['i_offsetvalue'.encode('gbk')])
                cjo['algorithm'] = Nonetonumber(holeconfig_dict['algorithm'.encode('gbk')])
                ja.append(cjo)
        jo['holeconfigHash'] = ja

        ja = []
        sql = ("select * from base_kcconfig where name='%s' order by id" % (fjo['开槽配置'.encode('gbk')].decode('gbk'))).encode('gbk')
        cur.execute(sql)
        ...
        ja = []
        #尺寸判定
        sql = ("select * from base_bomstd_rule where myclass1='%s' and myclass2='%s' order by [level] asc, [ID] asc" % (fjo['pname'].decode('gbk'), fjo['name'].decode('gbk'))).encode('gbk')
        cur.execute(sql)
...
def ImportXomItemForBom(param,AllPrice):
    ...
    if (string == '趟门,趟门') or (string == '掩门,掩门'):
        log.info(string.encode('gbk'))
        node = root.childNodes
        for ks in range(0, node.length):
            cnode = node[ks]
            if cnode.nodeName == '模板':
                cchildxml = ''
                ccnode = getfirstchild(cnode)
                if ccnode:
                    cchildxml = ccnode.toxml('Utf-8')
                AllPrice["模板"] = cchildxml
                if (string == '趟门,趟门'):
                    AllPrice["趟门门洞"] = readMDXml(cnode, AllPrice, 1)
                    obj = fun(cchildxml, AllPrice["趟门门洞"]["门洞宽"],
                ...
                if (string == '掩门,掩门'):
                    ...
                    obj = fungetdoorjson(cchildxml, AllPrice["掩门门洞"]["门洞宽"],
                    ...
                    if obj:
                        ...
                        ImportDoorsXomItemForBom(id - 1, param['cid'],
                                                 ...
                                                 param['extra'], param['mark'],
                                                 param['outputtype'],
                                                 cchildxml, ccnode.toxml('Utf-8'), obj)
                return

    if root.hasAttribute('板材单价'):
        string = root.getAttribute('板材单价')
        if string and param['pricecalctype'] == '':
            param['pricecalctype'] = string

    Var = {}
    value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm = 0, 0, 0, 0, 0, 0, 0, 0
    CurBL = []
    pData = [
        {  # // 把BH 放再最前面，避免在替换值的时候出现 替换H 值的时候把BH 也替换掉
            "值": Ord2PObj['BH'],
            "名称": "BH",
            "是否显示": "1",
            "Type": "变量"
    ...
    for i in range(0, node.length):
        if node[i].nodeName == 'BDXML':
            for j in range(0, node[i].childNodes.length):
                cnode = node[i].childNodes[j]
                if cnode.nodeType == 1:
    ...
    node = root.childNodes
    for i in range(0, node.length):
        if node[i].nodeName == '变量列表':
            #AllABl = []
            for j in range(0, node[i].childNodes.length):
                cnode = node[i].childNodes[j]
                if cnode.nodeType == 1 and cnode.nodeName == '变量':
                    #aBl = {}
                    vname = ''
                    if cnode.hasAttribute('名称'):
                        vname = cnode.getAttribute('名称')
                        #aBl[u'名称'] = vname
                    value = ''
                    if cnode.hasAttribute('值'):
                        value = cnode.getAttribute('值')
                        #aBl[u'值'] = value
                    if cnode.hasAttribute('是否显示'):
                        IsDisplay = cnode.getAttribute('是否显示')
                        #aBl[u'是否显示'] = IsDisplay
                    #aBl[u'Type'] = u"变量"
                    Var, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm = SetSysVariantValue(
                        vname, value, Var, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm,
                        value_rtm)
                    #AllABl.append(aBl)
            #AllPrice[u"变量列表"] = AllABl
    node = root.childNodes
    for i in range(0,node.length):
        if node[i].nodeName == '我的模块':
            mkJs = []
            for j in range(0,node[i].childNodes.length):
                cnode = node[i].childNodes[j]
                ...
                newpoi['装饰类别'] = textureclass
                newpoi['textureclass'] = textureclass
                newpoi['number_text'] = ''
                ...
                for k in range(16):
                    newpoi['var_args'][k] = 0
                    newpoi['var_names'][k] = ''
                #InitVarArgs(newpoi['var_args'],newpoi['var_names'])
                ...
                #if newpoi['name'] == u'QS-03':
                    #print 'x:', ToValueInt,Delphi_Round(SetSubject(varstr)), param['px'], int(param['px']), int(newpoi['lx']), newpoi['guid'], xx0
                varstr = ''
                ...
                if cnode.hasAttribute('SizeProgram'):
                    ...
                    LUAFILE = RootPath.decode('gbk') + '\\Program\\'
                ...
                SetSysVariantValueForOrderItem(varstr, newpoi, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm)
                ...
                if program_str:
                    ImportCloneItemForBom(Var, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm, program_str, newpoi, cnode, param, id, slino,AllPrice, mkJs)
...
def InitRootPathConfig(Path):
    ...
    #路径
    dataDBfile = RootPath.decode('gbk') + '\\Plugins\\data\\stanley.mdb'
    ...
    print('len of gBGHash1=', len(gBGHash))
    print('len of gPluginsList1=', len(gPluginsList))
    print('len of seqInfoHash1=', len(seqInfoHash))
    print('len of classseqInfoHash1=', len(classseqInfoHash))
    print('len of workflowlist1=', len(workflowlist))
    print('len of gBoardMatList1=', len(gBoardMatList))

    print('len of gErpItemList1=', len(gErpItemList))
    print('len of mIIHoleCalcRule1=', len(mIIHoleCalcRule))
    print('len of mDoorPrecision1=', mDoorPrecision)
    print('len of gROC1=', gROC.bj_out_classname)
    print('len of mJoBarCode1=', len(mJoBarCode))

def LoadXML2Bom(xmlfile, Path):
    global cid,bomlist,id,slino,mProductList, doorslist, slidinglist, JsonPrice,\
    des2wuliaodict,holeconfigHash,childbomHash,wjruleHash,mBDXMLList,\
    ruleHash,kcconfigHash,childxml,mTmpExp,mXMLStringList,mDoorList,bomstdList, \
    desdoorlist, desslidinglist, RootPath, des2price, QuoruleHash, classHash
    des2price = []
    QuoruleHash = {}
    classHash = {}
    RootPath = Path
    ...
    mProductList, bomlist, doorlist, slidinglist, mXMLStringList, mDoorList = [], [], [], [], [], []
    desdoorlist, desslidinglist = [], []
    mBDXMLList = {}
    productdatadict = {}
    JsonPrice = {"柜体列表": []}
    childxml = {}
    mTmpExp = {}
    #DOMTree = minidom.parseString('<?xml version="1.0" encoding="utf-8"?>'+xml)
    ...
    ImportXomItemForBom(param, AllPrice)
    #print json.dumps(AllPrice,encoding='utf8', ensure_ascii=False).encode('gbk')
    JsonPrice['柜体列表'].append(AllPrice)
    #print json.dumps(JsonPrice,encoding='utf8', ensure_ascii=False).encode('gbk')
    # j = j+1
    #Jsondata["JsonPrice"] = JsonPrice
    ## len(blist),id,json.dumps(blist,ensure_ascii=False)
    #// 拆分数量
...
def FunctionAddProgram1(productPart,param,cnode,mkJs):  # 第七步骤 添加ProgramStr属性 递归中添加
    ...
    if 'Program' in productPart:
        ...
        LUAPath = RootPath.decode('gbk') + '\\Program\\'
...
def Xml2ChildNodes(xml):
    ...
    node = DOMTree.documentElement
    return node
def ImportCloneItemForBom(Var, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg, value_ltm, value_rtm, program_str, clone_oi, clonenode, param, id, slino, AllPrice, mkJs):
    def trim(S):
        L = len(S)
        I = 0
    ...
    def UpdateAttribute(node, name, value):
        ...
        node.setAttribute(name.decode('utf8'), value)
    ...
    if not os.path.exists(luafile):
        ...
        while n > 0:
            ...
            SetSysVariantValueForOrderItem(varstr, newpoi, value_lsk, value_rsk, value_zk, value_zs, value_ls, value_lg,
                                           value_ltm, value_rtm)
...
if __name__ == '__main__':
    ...
    xmlfile = base_dir+'\\Python\\postxml\\1.xml'
    ...
    print('len of mDoorList=', len(mDoorList))
    print('len of doorslist=', len(doorslist))
    print('len of bomstdList=', len(bomstdList))
