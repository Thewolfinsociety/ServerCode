...
def ReturnDict(mDB, pKey):
    ...
    for i in range(0,len(pKey)):
        ...
        p = None
    return s
def CallGetXmlDes(mSceneXML, factoryid, outdir, DBID):

    tyconfigurl = 'http://127.0.0.1:8003/Price/ReturnBomConfig/'
    d = {'xml':mSceneXML, 'rootname':factoryid}      #string: xml, rootname 为工厂id
    ...
    if dataobj['result'] == 1:
        ...
        mSceneXML = mSceneXML.encode('gbk')
        ...
        desjson = json.loads(TYAndDesdatajson.decode('gbk'))
        return desjson['bomdeslist']

def CallGetXmlDes_old(configjson, mScendXML, outDespath):
...
def CallXml2JsonBom(mSceneXML, outdir, bomdeslist, DBID):
    mSceneXML = mSceneXML.encode('gbk')

    desurl = 'http://127.0.0.1:8002/Qdbom/AllDesData/'
    globalurl = 'http://127.0.0.1:8002/Qdbom/GlobalData/'
    d = {'deslist': json.dumps(bomdeslist, ensure_ascii=False), 'factorypath': factoryid}
...
def CallXml2JsonBom_old(mScendXML, outBompath):
    mScendXML = mScendXML.encode('gbk')
    config = {
        'gtconfig':{
                'username': '学习',
...
def TestOrdToXmlString(ordfile):
    ...
    for DBID in ProDBIDList:
        ...
        version = r.get('018' + 'Verison')
        if not version:
            r.set('018' + 'Verison', '0.01')
            version = '0.01'
        myversion = '018' + version.decode('utf8')
        if flag == 1:
            # configdata = LoadXML2Bom(ourpath, myversion, "D:\\nginx-1.0.11\\nginx-1.0.11\html\\ServerData\\018")
            ...
            CallGetXmlDes(string.decode('utf8'), factoryid, outdir, DBID)
        elif flag ==2:
            # configdata = LoadXML2Bom(ourpath, myversion, "D:\\nginx-1.0.11\\nginx-1.0.11\html\data")
            # outDespath = os.path.join(outdir, 'outDesdata_' + DBID)
            bomdeslist = CallGetXmlDes(string.decode('utf8'), factoryid, outdir, DBID)
            #outbomdatapath = os.path.join(outdir, 'outBomdata_' + DBID)
            CallXml2JsonBom(string.decode('utf8'), outdir, bomdeslist, DBID)
        else:
