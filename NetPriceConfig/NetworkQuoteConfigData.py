...
# 获取计价配置参数信息
# 2020-03-18
def getPriceConfigMSG(cur):
    '''
    :return:        计价配置参数
    '''
    sql = "select dataFormat, ExcelPath from priceConfig"
    cur.execute(sql)
    u = cur.fetchone()
    if u[0] == 0:
        priceType = u"默认"
        ...
        priceType = u"格式2"
    dict = {"报价数据格式": priceType, "导出Excel路径": u[1]}
    return dict
...
def GetNetworkQuoteConfigHandl(name, slidingDoor, swingDoor, rootPath, myversion):
    ...
    try:
        ...
        priceConfig = getPriceConfigMSG(cur)  # 计价配置
        ...
        dict = {"材料": materials,    #1
                "板件折扣方案": boardDisScheme,  #1
                "报价排序": quoteSort,   #1
                "基础五金": baseWj,      #1
                "用户权限": userPermission,    #1
                "合并规则": mergingRule,    #1
                "报价方案": QuoteScheme,
                "报价配置": array,
