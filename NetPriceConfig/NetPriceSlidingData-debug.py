...
def returnresdata(recemessage, r):
    #try:
    status = True
    ...
    result = json.loads(content.decode('gbk'), encoding='gbk')
...
def getdata(rootname, attri, func, cur, r):
    status, result = returnresdata(rootname + attri, r)
    if status:
        print('redis yes', rootname + attri)
        mPExp = result
    else:
        print('redis no',rootname + attri)
        mPExp = func(attri, cur)  # 单门数量类型
...
def getu(n, cur):
    ...
    # sql = ("select name2, price, price2, price3, jgprice, jgprice2, jgprice3,\
    #                     color, discount, pricetype, myunit from TM_parts")
    # cur.execute(sql)
    # u = cur.fetchall()
    # print (n, u)
    return u

def get_TM_parts(name, cur, myversion, r):
    '''
    配件数据提取
    :param name: 配件类型
    :return:     相关配件数据
...
'''根据name获取趟门相关数据'''
def GetDoorDataHandler(name, rootPath, myversion, r):
    '''
    根据name获取趟门相关数据
    :param name:
    :return: 趟门数据
    ...
    array = []
    '''配件'''
    data = get_TM_parts(name, cur, myversion, r)
    if data:
        array += data
    '''门板'''
