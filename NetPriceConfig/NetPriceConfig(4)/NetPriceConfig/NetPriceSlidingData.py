...
def get_TM_doorPlank(name, cur):
    ...
    for n in name:
        sql = ("select name1, name2, direct, price, price2, price3, jgprice, "
               ...
               "lvalue, wmax, wmin, wvalue, hmax, hmin, hvalue, discount, pricetype,"
               " myunit, quoteType, quoteType "
        ...
        if u:
            for a in u:
                dict = {"myType": "门板", "name1": a[0], "name2": a[1], "纹路": a[2],
                        ...
                        "折扣类型": a[23], "报价类型": a[24], "单位": a[25], "报价方式": a[26], "计算防撞条": a[27]}
                if dict not in array:
                    array += [dict]
...
def get_TM_doorOpening(name, cur):
    ...
    for n in name:
        sql = ("select name1, price, price2, price3, jgprice, jgprice2, jgprice3,\
                ...
                discount, pricetype, myunit, overlap from TM_doorOpening where name1=?")
        sdata = (n, )
        cur.execute(sql, sdata)
        u = cur.fetchall()
        if u:
            for a in u:
                dict = {"myType": "门洞", "name2": a[0],
                        ...
                        "折扣类型": a[17], "报价类型": a[18], "单位": a[19], "计算重叠位": a[20]}
                if dict not in array:
                    array += [dict]
    return array
