...
def GetQuoteScheme(data, cur):
    ...
    for i in data:
        ...
        sql = ("select quote_group, quote_scheme, supply_price, supply_process_charges,"
               ...
               "sell_process_charges, retail_price, retailFormula, retail_process_charges, "
               "color, AreaFormula, MinWidth, MaxWidth, MinDepth, MaxDepth "
               "from quote_scheme where quote_group=? and quote_scheme=?")
        data = (name[0], name[1])
        cur.execute(sql, data)
        u = cur.fetchall()
        list = []
        if not u:
            ...
            for a in u:
                dict = {"组别": a[0], "报价方案": a[1], "供货价格": a[2], "供货加工费": a[3],
                        ...
                        "颜色": a[15], "面积计算公式": a[16],
                        "最小宽度": a[17], "最大宽度": a[18], "最小深度": a[19], "最大深度": a[20],
                        "门报价": doorQuote}
                list += [dict]
                array[i] = list
    return array
...
# 根据类别列表获取网络报价配置
def GetNetworkQuoteConfigHandl(name, slidingDoor, swingDoor, rootPath):
    print("-----网络报价配置数据获取---")
    try:
        SlidingThread = threading.Thread(target=NetPriceSlidingData.GetDoorDataHandler, args=(slidingDoor, rootPath))
        SlidingThread.start()
        SwingThread = threading.Thread(target=NetPriceSwingData.GetSwingNetConfig, args=(swingDoor, rootPath))
        SwingThread.start()
        SlidingThread.join()

        # slidingDoorData = NetPriceSlidingData.GetDoorDataHandler(slidingDoor, rootPath)  # 趟门配置数据
        # swingDoorData = NetPriceSwingData.GetSwingNetConfig(swingDoor, rootPath)  # 掩门配置数据
        DBfilename = rootPath + "\\data\\" + "NetPriceDB.db"
        conn = sqlite3.connect(DBfilename)
        cur = conn.cursor()
        materials = GetMaterialsData(cur)          # 材料表
        boardDisScheme = GetBoardDisScheme(cur)    # 板件折扣
        userPermission = GetUserPermission(cur)    # 用户权限
        mergingRule = GetMergingRules(cur)         # 合并规则
        quoteSort = GetQuoteOutputSort(cur)        # 报价输出排序
        baseWj = getBase_Wj(cur)                   # 基础五金
        ...
        result = {"state": -1, "info": "获取网络报价配置数据系统错误"}
        return result
