...
# from PythonToSrc.xmltojson import functionxmltoSrc
#from PythontoBomJson.xmltojson import XmltoJson

# from PythontoBomJson.useful_tools import *
# from PythontoBomJson.GlobalInitData import defineRootPath
...
log = Logger(base_dir+"\\Python3\\Log\\all.log", level='info')
sys.path.append(webpath+'\\Python3\\PythontoBomJson')

class DbDataBase(object):
    def __init__(self, filepath, qdsoft):
        self.path = filepath
        self.databasename = qdsoft
        self.createdbfile()
    def createdbfile(self):
        self.conn = sqlite3.connect(self.path)
        cur = self.conn.cursor()

        cur.execute('''CREATE TABLE IF NOT EXISTS DoorConfigData
                           (qdsoft    CHAR(255),
                            groupnum     CHAR(255),
                            key       CHAR(255),
                            name      CHAR(255) PRIMARY KEY unique,
                            data    TEXT);''')
        ...
        if content is None:
            ...
            for c in content:
                if c == '"':
                    ...
                    string += c
            return string
    def isexistname(self, group, qdsoft, key, configdata):
        sql = "select * from DoorConfigData where name = '{}'".format(str(group) + '_' + qdsoft + '_' + key)
        cur = self.conn.cursor()
        cur.execute(sql)
        alldatalist = cur.fetchall()
        if len(alldatalist) >= 1:
            #MySQLdb.escape_string(configdata).decode('utf8')
            # sql = 'UPDATE DoorConfigData SET data ="' +MySQLdb.escape_string(configdata).decode('utf8') + '" where name = "{}"'.format(str(group) + '_' + qdsoft + '_' + key)
            try:
                sql = "UPDATE DoorConfigData SET data = '{}' where name = '{}'".format(quote(configdata, 'utf-8'), str(group) + '_' + qdsoft + '_' + key)
                cur.execute(sql)
                self.conn.commit()
                print('update')
            except:
                print('delete')
                sql = "DELETE FROM  DoorConfigData where name = '{}'".format(str(group) + '_' + qdsoft + '_' + key)
                cur.execute(sql)
                self.conn.commit()
                sql_backpart = "(qdsoft, groupnum, key, name, data) VALUES (?,?,?,?,?)"
                data = (qdsoft, str(group), key, str(group) + '_' + qdsoft + '_' + key,
                        MySQLdb.escape_string(configdata))
                sql = "INSERT INTO DoorConfigData" + sql_backpart
                cur = self.conn.cursor()
            ...
            pass
        return False
    def SetItem(self, group, qdsoft, key, configdata):
        if self.isexistname(group, qdsoft, key, configdata):
            return
        sql_backpart = "(qdsoft, groupnum, key, name, data) VALUES (?,?,?,?,?)"
        data = (qdsoft,str(group), key, str(group)+'_'+qdsoft+'_'+key, quote(configdata, 'utf-8')) #MySQLdb.escape_string(configdata)
        sql = "INSERT INTO DoorConfigData" + sql_backpart
        cur = self.conn.cursor()
        cur.execute(sql, data)
        self.conn.commit()
    ...
    def DeleteItem(self, keylistStr):
        cur = self.conn.cursor()
        sql = "DELETE FROM  DoorConfigData where name in {}".format(keylistStr)
        cur.execute(sql)
        self.conn.commit()
        self.conn.close()
        return 1
    def DeleteOneItem(self, keylistStr):
        cur = self.conn.cursor()
        sql = "DELETE FROM  DoorConfigData where name = '{}'".format(keylistStr)
        cur.execute(sql)
        self.conn.commit()
        self.conn.close()
        return 1
    def GetItem(self, group, qdsoft, key):
        sql = "select * from DoorConfigData where name = '{}'".format(str(group)+'_'+qdsoft+'_'+key)
        cur = self.conn.cursor()
        cur.execute(sql)
        alldatalist = cur.fetchall()
        ...
        if len(alldatalist) >= 1:
            ...
            pass
        return Result
    def GetKeys(self, group, qdsoft):
        sql = "select key from DoorConfigData where groupnum = '{}' and qdsoft='{}'".format(group, qdsoft)
...
def DoorInitData1(adb, rootname, Path):
    RootPath = Path
    try:
...
def SlidingInitData1(adb, rootname, Path):
    RootPath = Path
...
def AddDataToDBD(myversion):
    ...
    #print (r.get(myversion + 'mSlidingExpList'))
    #print(mExpListdata.decode('utf8'))
    if not mExpListdata:
...
class ReturnConfig(BaseHandler):
    ...
    def post(self):
        ...
        if not version:
            r.set(RootName + 'Verison', '0.01'.encode('utf8'))
            version = '0.01'.encode('utf8')
        myversion = RootName + version.decode('utf8')
        #print(myversion)
        AddDataToDBD(myversion)
        # 本地测试版
        xmlobj = {}
        xmlobj['xml'] = xml.decode('utf8')
        xmlobj['RootName'] = myversion
        xmlobj['RootPath'] = ServerPath + RootName
        m2 = hashlib.md5()
        m2.update(xml)
        pricedatamd5 = m2.hexdigest()
        log.logger.debug('pricedatamd5=' + pricedatamd5)
        ...
        #     with open(RootPath + '\\Python\\ConfigWebcache\\'+pricedatamd5,'r') as f:
        #         Bomjson = f.read()
        #         self.write(Bomjson)
        # else:
        UpLoadXmlPath = RootPath + '\\Python\\UpLoadXml\\'
        ...
        # 本地测试版
        configjson = LoadXML2Bom(UpLoadXmlPath + guid, myversion, RootPath)
        # config['result'] = 1
        ...
        with open(RootPath + '\\Python\\ConfigWebcache\\' + pricedatamd5, 'w+', encoding='utf8') as f:
            f.write(configjson.decode('utf8'))
        self.write(configjson)
        os.remove(UpLoadXmlPath + guid)
        # xmljson = json.dumps(xmlobj, ensure_ascii=False)
        # taskid = yield self.calresult(xmljson)
        # Result['taskid'] = taskid
        # Result['result'] = 1
        # self.finish(json.dumps(Result, ensure_ascii=False).encode('utf8'))

    @tornado.gen.coroutine
    def calresult(self, xmljson):
        #print("begin calculate:sum %d+%d" % (a, b))
        #future = Future()
        self.remote_call_result = tasks.calxml.apply_async(args=[xmljson])
        # print(self.remote_call_result.id)
        # print(self.remote_call_result.ready())
        #time.sleep(2)
        # while not self.remote_call_result.ready():
        #     #print(self.remote_call_result.ready())
        #     yield tornado.gen.sleep(0.001)
        # #print(self.remote_call_result.ready())
        # def callback(xmljson,pricedatamd5,UpLoadXmlPath,guid):
        #     #configjson = LoadXML2Bom(UpLoadXmlPath + guid, RootName, RootPath)
        #     # config['result'] = 1
        #     #print(self.remote_call_result.ready())
        #
        #     #     yield tornado.gen.sleep(0.05)
        #     #async1 = AsyncResult(self.remote_call_result.id)
        #     # # while not async1.ready():
        #     # #     yield tornado.gen.sleep(0.05)
        #     configjson = self.remote_call_result.get()
        #     #print(configjson)
        #     #configjson = json.dumps(config, ensure_ascii=False).encode('utf8')
        #     # # PriceJsonPath = base_dir + u'\\Python\\PythontoBomJson\\PriceJson\\'
        #     # if not os.path.exists(RootPath + '\\Python\\ConfigWebcache\\'):
        ...
        #     # # self.finish()
        #     # os.remove(UpLoadXmlPath + guid)
        #     future.set_result(configjson)
        #
        # tornado.ioloop.IOLoop.instance().add_callback(callback, xmljson, pricedatamd5, UpLoadXmlPath,guid)
        #
        # result = yield future
        #print('result=', result)
        # print("after yielded")
        # print("the %d+%d=%d" % (a, b, result))
        #configjson = json.dumps(result, ensure_ascii=False).encode('utf8')
        result = self.remote_call_result.id
        # self.write(self.remote_call_result.id)
        return result

    def awake(self,RootPath, config, pricedatamd5, UpLoadXmlPath, guid):
        config['result'] = 1
        configjson = json.dumps(config, ensure_ascii=False).encode('utf8')
        # PriceJsonPath = base_dir + u'\\Python\\PythontoBomJson\\PriceJson\\'
        if not os.path.exists(RootPath + '\\Python\\ConfigWebcache\\'):
...
class ReturnConfig_Celery(BaseHandler):
    @tornado.gen.coroutine
    def post(self):
        global RootPath
        Result = {'result': 0}
        ...
        if not version:
            ...
            version = '0.01'.encode('utf8')
        myversion = RootName + version.decode('utf8')
        AddDataToDBD(myversion)    #如果缓存中不存在数据库，即从本地数据库更新至redis
        # 本地测试版
        xmlobj = {}
        xmlobj['xml'] = xml.decode('utf8')
        xmlobj['RootName'] = myversion
...
class CalStateHandle_Celery(BaseHandler):
    def post(self):
        task_id = self.get_argument('task_id', '')
        if task_id == "":
            self.write(json.dumps({'result':-1}).encode('utf8'))
            return
...
class ReturnPriceConfig(tornado.web.RequestHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        # print('slidingDoor=' + slidingDoor.decode('utf8'))
        # print('swingDoor=' + swingDoor.decode('utf8'))
        # print('rootname=' + RootName)
        # print('rootname=' + RootName)
...
class DoorSetDataHandle(BaseHandler):
    def post(self, *args, **kwargs):
        group = self.get_argument('group')
        qdsoft = self.get_argument('qdsoft')
        key = self.get_argument('key')
        configdata = self.get_argument('data')
        DoorDb = DbDataBase(DataBasesFilePath, qdsoft)
        DoorDb.SetItem(group, qdsoft, key, configdata)
        Res = {'result': 1}
        self.write(json.dumps(Res, ensure_ascii=False).encode('utf8'))

class DoorGetDataHandle(BaseHandler):
    def post(self, *args, **kwargs):
        group = self.get_argument('group')
        qdsoft = self.get_argument('qdsoft')
        key = self.get_argument('key')
        DoorDb = DbDataBase(DataBasesFilePath, qdsoft)
        alldatalist = DoorDb.GetItem(group, qdsoft, key)
        if alldatalist:
            Res = {'configdata': unquote(alldatalist[4], 'utf-8')}
            Res['result'] = 1
            self.write(json.dumps(Res, ensure_ascii=False).encode('utf8'))
        else:
            Res = {'result': 0}
...
class DoorDeleteDataHandle(BaseHandler):
    def post(self, *args, **kwargs):
        group = self.get_argument('group')
        qdsoft = self.get_argument('qdsoft')
        keylistJson = self.get_argument('key')
        keylist = json.loads(keylistJson)
        # print(keylist)
        # print([(str(group) + '_' + qdsoft + '_' + key) for key in keylist])
        # print(len(tuple([(str(group) + '_' + qdsoft + '_' + key) for key in keylist])))
        newkeylist = tuple([(str(group) + '_' + qdsoft + '_' + key) for key in keylist])

        if len(newkeylist) == 1:
            DoorDb = DbDataBase(DataBasesFilePath, qdsoft)
            DoorDb.DeleteOneItem(newkeylist[0])
            Res = {'result': 1}
            self.write(json.dumps(Res, ensure_ascii=False).encode('utf8'))
        else:
            DoorDb = DbDataBase(DataBasesFilePath, qdsoft)
            newkeylistStr = str(newkeylist)
            DoorDb.DeleteItem(newkeylistStr)
            Res = {'result':1}
            self.write(json.dumps(Res, ensure_ascii=False).encode('utf8'))


class DoorGetKeyHandle(BaseHandler):
    def post(self, *args, **kwargs):
        group = self.get_argument('group')
        qdsoft = self.get_argument('qdsoft')
        DoorDb = DbDataBase(DataBasesFilePath, qdsoft)
        alldatalist = DoorDb.GetKeys(group, qdsoft)
        Res = {}
        if alldatalist:
            keylist = [alldata[0] for alldata in alldatalist]
            Res['keylist'] = keylist
            Res['result'] = 1
            self.write(json.dumps(Res, ensure_ascii=False).encode('utf8'))
        else:
...
class EchoWebSocket(WebSocketHandler):
    ...
    def on_message(self, taskid):
        print('taskid',taskid)
        asyncres = AsyncResult(taskid)

        result = {'result':0}
        if asyncres.successful():
            resultstring = asyncres.get()

            result = {'result': 1, 'caltext':resultstring}
            print(result)
        else:
            print(result)

        ResultJson = json.dumps(result, ensure_ascii=False).encode('utf8')
        self.write_message(ResultJson)

    def on_close(self):
        print("WebSocket closed")

class DoorGetTextStringHandle(BaseHandler):
    def get(self, *args, **kwargs):
        group = self.get_argument('group')
        qdsoft = self.get_argument('qdsoft')
        key = self.get_argument('key')
        ...
        if alldatalist:
            ...
            #      f.write(unquote(alldatalist[4], 'utf-8'))
            self.write(unquote(alldatalist[4], 'utf-8'))
            #self.write(alldatalist[4].decode('utf8').replace('\\n','***').replace('\\','').replace('***','\\n'))
        else:
            self.write('')
...
application = tornado.web.Application([
    ...
    (r"/Price/ReturnBomConfig_Celery/", ReturnConfig_Celery),
    (r"/Price/CalState_Celery/",CalStateHandle_Celery),
    (r"/Price/ReturnBomConfig/", ReturnConfig),
    (r"/Price/ReturnPriceConfig/", ReturnPriceConfig),
    (r"/Price/GetTextData/", DoorGetDataHandle),
    (r"/Price/SetTextData/", DoorSetDataHandle),
    (r"/Price/GetTextKeyList/", DoorGetKeyHandle),
    (r"/Price/DeleteTextKey/", DoorDeleteDataHandle),
    (r"/Price/GetTextString/", DoorGetTextStringHandle),
    (r"/Price/mywebsocket/",EchoWebSocket),
...
if __name__ == '__main__':
    ...
    base_dir = os.path.abspath(os.path.join(os.getcwd(), '..'))
    DataBasesFilePath = base_dir + '\\Python\\sqlitedb\\DoorDb.db'
    #print('flag=',AddDataToDBD('data'))
    #AllworkQueue = Queue.Queue()  # 批量导单队列
