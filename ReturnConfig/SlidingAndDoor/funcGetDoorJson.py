...
def InitData():
    ...
    try:
        ...
        CoverTypeType = cf.get("CoverType", "nType")
    except:
        CoverTypeType = 0
...
def DoorInitData(Path):
    ...
    if os.path.exists(RootPath + '\\data\\Door\\doorHoleConfig.js'):
        with open(RootPath + '\\data\\Door\\doorHoleConfig.js',encoding='utf8') as f:
            jsdata = f.read()
            HoleConfig = jsdata
    else:
        HoleConfig = ''
...
def LoadFromXMLTemplate(xml, l, h):
    ...
    if mIsVertical != False and (
            mIsVertical == 'true' or mIsVertical == 'True' or mIsVertical == 'TRUE' or mIsVertical == True):
        mIsVertical = True
    mDoorMemo = root.get('DoorMemo', '')
    mExtend = root.get('Extend', '')
    if mExtend != '': mExtend = urllib.parse.quote(mExtend.encode('gbk'))
        ...
        for i in range(len(mWJBomDetailList)):
            ...
            if ((pwjbomdetail['bomname'] == pwjbom['name']) and ((pwjbomdetail['door_bh'] == 0)
                ...
                if (pa):
                    ...
                    if ((pa['bomtype'] == '木板') or (pa['bomtype'] == '玻璃') or (pa['bomtype'] == '百叶') or (
                            pa['bomtype'] == '板材')):
                        pwl['bomtype'] = '板材'
            ...
            if (attri):
                ...
                if (attri):
                    ...
                    if (attri):
                        ...
                        if ((pwl['bomtype'] == '木板') or (pwl['bomtype'] == '玻璃') or (pwl['bomtype'] == '百叶') or (
                                pwl['bomtype'] == '板材')):
                            pwl['bomtype'] = '板材'
...
def GetHingeName(phinge, ct):
    ...
    for i in range(0, 99):
        ...
        t = jo['T' + str(i + 1)]
        zn = jo['Zn' + str(i + 1)]
        if (zn == 'True' or zn == "TRUE" or zn == True or zn == 1):
            znmj = 1
        else:
            znmj = 0
...
def GetHboxKCInfo(door, n, p):
    def GetNearestDownPanel(p):
        ...
        for i in range(0, len(door['panellist'])):
            pnl = door['panellist'][i]
            if (p == pnl): continue
            if (((Number(p['y0']) + Number(p['h0']) - pnl['y0']) > 0) and (
                    (Number(p['y0']) + Number(p['h0']) - pnl['y0']) < t)):
...
#     if (HoleConfig):
#         # log.debug('mHingeHoleDes='+str(mHingeHoleDes)+','+'l='+str(l)+','+
...
def GetHingeHoleBDEx(mHingeHoleDes, l, mOpenDirect, mHingeHoleParam):
    if (HoleConfig):
        ...
        Result = jscontext.call('GetHingeHoleBD', mHingeHoleDes, l, mOpenDirect, mHingeHoleParam)

        return Result
def GetHandleHoleBDEx(sign, mHandleW, mHandleH, mOpenDirect):
    if (HoleConfig):
        # log.debug('sign=' + str(sign) + ',' + 'mHandleW=' + str(mHandleW) + ',' +
...
def GetJsonFaceB(door, direct):  # // -10190
    ...
    if sign != '':
        try:
            ja = ctxt.eval('var s = GetHandleHoleBD("%s","%s","%s","%s");' % (
            sign, door['mHandleW'], door['mHandleH'], door['mOpenDirect']))
            ja = ctxt.eval('var sjson = JSON.stringify(s);')
            jajson = str(ctxt.eval('sjson'))
...
def GetXMLBom(xml, mL, mH):
    ...
    for i in range(0, len(mDoorsList)):
        ...
        if (mPType['isframe']):
            if (mPParam['iscalc_framebom'] == True):  # // 计算横框竖框物料
                ...
                pwl['memo'] = (pwl['memo'].encode('utf8')).replace('$竖框槽位信息', GetVboxKCInfo(door))
                ...
                pwl2 = copy.deepcopy(pwl)
                pwl['memo'] = mPParam['udmemo']
                pwl['memo'] = (pwl['memo'].encode('utf8')).replace('$横框槽位信息', GetHboxKCInfo(door, 2, None)).decode(
                    'utf8')
            ...
            for j in range(0, len(door['panellist'])):
                ...
                if (pnltype):
                    ...
                    GetPanelBom(ALLlist, pnltype['panelbom'], pnl['类型'], pnl['颜色'], pnl['颜色2'], mMyVBoxColor, pnl['w1'],
                                pnl['h1'])
    ...
    for i in range(0, len(ALLlist)):
        ...
        newpw2["H"] = ''
        if pwl['w'] != '':
            newpw2["W"] = str(int(Number(pwl['w']))) if int(Number(pwl['w'])) == float(pwl['w']) else str(
                float(pwl['w']))
        if pwl['l'] != '':
            newpw2["L"] = str(int(Number(pwl['l']))) if int(Number(pwl['l'])) == float(pwl['l']) else str(
                float(pwl['l']))
        if pwl['h'] != '':
            newpw2["H"] = str(int(Number(pwl['h']))) if int(Number(pwl['h'])) == float(pwl['h']) else str(
                float(pwl['h']))
        newpw2["#"] = j
        ...
        if newpw2["BDFILE"] != '' and os.path.exists(RootPath.decode('gbk') + '\\' + newpw2["BDFILE"]):
            ...
            # log.debug('text='+text.encode('gbk'))
...
def BD2XML1(xml, newpw2):
    ...
    if 'FaceA' in newpw2 and newpw2['FaceA'] != '':
        ...
        for i in range(0, len(ja)):
            ...
            if (cjo['Face'] == 'A'):
                nodeA.appendChild(cnode)
            elif (cjo['Face'] == 'B'):
                nodeB.appendChild(cnode)
            else:
                cnode = None
                continue
            if 'X' not in cjo:
