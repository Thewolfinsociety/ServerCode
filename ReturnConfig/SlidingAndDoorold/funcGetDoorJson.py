...
def InitData():
    ...
    try:
        cf = copyini(base_dir + '\\QdCloud\\XDiyDoors.ini')
        CoverTypeType = cf.get(u"CoverType".encode('gb2312'), u"nType".encode('gb2312'))
    except:
        CoverTypeType = 0


    with open(base_dir + '\\QdCloud\\Door\\doorHoleConfig.js', 'r') as f:
        jsdata = f.read().decode('utf8')
        HoleConfig = jsdata
    try:

        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\五金配件.cfg'.encode('gb2312'),'r') as f:
    ...
    try:
        ...
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\颜色.cfg'.encode('gb2312'),'r') as f:
    ...
    try:
        ...
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\颜色分类.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        ...
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\颜色分类2.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        ...
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\门类型.cfg'.encode('gb2312'),'r') as f:
    ...
    try:
        ...
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\单门数量类型.cfg'.encode('gb2312'),'r') as f:
    ...
    try:
        ...
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\拉手.cfg'.encode('gb2312'),'r') as f:
    ...
    try:
        ...
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\中横框参数.cfg'.encode('gb2312'),'r') as f:
        ...
        mHBoxParamListcontent = json.dumps([])
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\门铰分类.cfg'.encode('gb2312'),'r') as f:
    ...
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\门铰.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\门芯附加物料.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\门芯类型.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\掩门参数.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\报价.cfg'.encode('gb2312'), 'r') as f:
        ...
        mPriceListcontent = json.dumps([])
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\报价方案.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\百叶板配置.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\五金配件分类.cfg'.encode('gb2312'), 'r') as f:
        ...
        mWJBomListcontent = json.dumps([])
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\五金配件分类数据.cfg'.encode('gb2312'), 'r') as f:
    ...
    try:
        with open(base_dir + u'\\QdCloud\\Door\\掩门配置表\\XML单门结构.cfg'.encode('gb2312'), 'r') as f:
...
def fungetdoorjson(xml,W,H, mGetHingeHoleBD, mGetHandleHoleBD):
...
def TDoorRect(mDoorsList,i):
    ...
    doo['x1'] = mDoorsobject[u'X1']
    doo['y1'] = mDoorsobject[u'Y1']
    ...
    if (node):
        ...
        for i in range(0, len(node)):
            ...
            if (attri):
                ...
                ll = SAndDoorSetSubjectattri(attri, mExp)
                ll = ToValueFloat(ll)

                attri = cnode.GET(u'深', '0')
                if (attri):

                    dd = SAndDoorSetSubjectattri(attri, mExp)
                    dd = ToValueFloat(dd)

                    attri = attri = cnode.GET(u'高', '0')
                    if (attri):

                        hh = SAndDoorSetSubjectattri(attri, mExp)
                        hh = ToValueFloat(hh)
...
def GetXMLBom(xml, mL, mH):
    ...
    for i in range(0, len(mDoorsList)):
        ...
        if ((door1 != None) and (mPParam['left_doorxml'] != '') and (door['mOpenDirect'] == u'左') and (
                ...
                GetParamXMLBom(ALLlist, mPParam['doorxml'], i + 1, door['x1'] - mLCap,
                               oor['y1'] - mDCap, door['doorw'], door['doorh'])
        ...
        if (mPType['isframe']):
            ...
            for j in range(0, len(door['panellist'])):
                ...
                if (mytype == u'百叶'):
                    ...
                    h = Number(pnl['h1'])
                    w = Number(pnl['w1'])
                    # print 'pnl=',json.dumps(pnl,ensure_ascii=False)
                    # print 'w=',Number(pnl['w1']),pnl['w1']
                    t = 0
    ...
    for i in range(0, len(ALLlist)):
        ...
        if newpw2["BDFILE"] !='' and os.path.exists(base_dir.decode('gbk') +'\\' +newpw2["BDFILE"]):
            with open(base_dir.decode('gbk') +'\\' +newpw2["BDFILE"], 'r') as f:
                xml = f.read()
...
xml4 = '''<掩门 guid="E31530107022B0B5310C6B40514192E4" 门洞宽="1064" 门洞高="886" L门缝="0.0000" H门缝="0.0000" M门缝="-1.0000" 单门数量类型="双掩门" 门类型="木框门" 门颜色="请选择颜色" 均分="0" 左盖="18" 右盖="18" 上盖="18" 下盖="18" ZNMJ="0" CT="0" 门框类型="中纤板(QS-01A)" 门框颜色="845B#胡桃木" 中横框类型="QS-01A门中横框" 中横框颜色="845B#胡桃木" 门芯类型="经典百叶(横放)" 门芯颜色="70#象牙白" 是否竖排="False" 是否带框="1" DoorMemo="" Extend="" HingeHole="" DataMode="0" Extra="{^path^:^QdCloud/Door/product/平开门/QS-01A^,^productName^:^QS-01A^}">
                  <分格 面板高度="817" 高度="817" 材料="经典百叶(横放)" 颜色="70#象牙白" 纹路="横纹" 胶条位="" 备注="" 定格="0"/>
                  <单门 宽="547.5" 高="919" DW1="550" DH1="922" X0="0.00" Y0="0" X1="0.00" Y1="0" 门框类型="中纤板(QS-01A)" 打开方向="左" 拉手="623-S" 门铰="东泰铰链@#MJ-D01内置阻尼铰链110直臂" 门铰CT="全盖" HandleX="500.5" HandleY="811" HandleW="34" HandleH="86" HandlePos="{^X^:13,^AL^:2,^Sign^:^^,^AL_Y^:22,^isCenter^:0}" HandlePosX="30" HandlePosY="0" VBOXH="70.00" UDBOXH="70.00" T_W="2.50" LKVALUE="3.00" Memo="" HingeHoleDes="01-18侧板门铰孔" HingeHoleParam="{^一^:0,^二^:0,^三^:0,^四^:0,^五^:0}" HingeHoleExtra="{^门铰^:[{^门铰类型^:^MJ-D01内置阻尼铰链110直臂^,^偏移值^:^0^},{^门铰类型^:^MJ-D01内置阻尼铰链110直臂^,^偏移值^:^0^},{^门铰类型^:^MJ-D01内置阻尼铰链110直臂^,^偏移值^:^0^}],^门铰孔标注^:^120-340-339-120^,^开门方向^:^左^}">
                    <门芯 类型="经典百叶(横放)" 颜色="70#象牙白" 颜色2="" 纹路="横纹" w0="407.5" h0="779" x0="70" y0="70" d0="18" w1="445.5" h1="817" x1="51" y1="51" d1="18" w2="547.5" h2="919" x2="0" y2="0" d2="18"/>
                  </单门>
                  <单门 宽="547.5" 高="919" DW1="550" DH1="922" X0="549.50" Y0="0" X1="550.00" Y1="0" 门框类型="中纤板(QS-01A)" 打开方向="右" 拉手="623-S" 门铰="东泰铰链@#MJ-D01内置阻尼铰链110直臂" 门铰CT="全盖" HandleX="13" HandleY="811" HandleW="34" HandleH="86" HandlePos="{^X^:13,^AL^:2,^Sign^:^^,^AL_Y^:22,^isCenter^:0}" HandlePosX="30" HandlePosY="0" VBOXH="70.00" UDBOXH="70.00" T_W="2.50" LKVALUE="3.00" Memo="" HingeHoleDes="01-18侧板门铰孔" HingeHoleParam="{^一^:0,^二^:0,^三^:0,^四^:0,^五^:0}" HingeHoleExtra="{^门铰^:[{^门铰类型^:^MJ-D01内置阻尼铰链110直臂^,^偏移值^:^0^},{^门铰类型^:^MJ-D01内置阻尼铰链110直臂^,^偏移值^:^0^},{^门铰类型^:^MJ-D01内置阻尼铰链110直臂^,^偏移值^:^0^}],^门铰孔标注^:^120-340-339-120^,^开门方向^:^右^}">
                    <门芯 类型="经典百叶(横放)" 颜色="70#象牙白" 颜色2="" 纹路="横纹" w0="407.5" h0="779" x0="619.5" y0="70" d0="18" w1="445.5" h1="817" x1="600.5" y1="51" d1="18" w2="547.5" h2="919" x2="549.5" y2="0" d2="18"/>
                  </单门>
                </掩门>'''
